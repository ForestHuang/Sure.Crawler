...
namespace Sure.Crawler.General
{
    public class DownloadProgressListener : IDownloadProgressListener
    {
        ...
        public void OnDownloadSize(long size)
        {
            ...
            //下载速度
            if (downMsg.Size == 0) { downMsg.Speed = size; }
            else { downMsg.Speed = (float)(size - downMsg.Size); }

            if (downMsg.Speed == 0) { downMsg.Surplus = -1; downMsg.SurplusInfo = "未知"; }
            else { downMsg.Surplus = ((downMsg.Length - downMsg.Size) / downMsg.Speed); }
